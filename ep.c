...
int main() {
    ...
    loop:
        ...
        if(DEBUG) for(int i = 0; i < nword; i++) printf("word[%d] = %s\n", i, word[i]);
        ...
        if(DEBUG) for(int i = 0; i < nfield; i++) printf("field[%d] %d x %d : %d v: %d\n", i, field[i].i, field[i].j, field[i].s, field[i].v);
        ...
        backtrack:
            ...
            if(DEBUG) showmaps(map, mapt, m, n);
            if(DEBUG) scanf("%c", &c);
