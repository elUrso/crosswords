    ...
    loop:
        ...
        backtrack:
            switch(stack[base]) {
                ...
                default: /*initialized*/
                    recheck:
                    for(; sword[base] != field[stack[base] /*pop t, push t*/].s && stack[base] < nfield; stack[base]++ /*pop t, push t+1*/);
                    if(stack[base] < nfield && valid(map, mapt, m, n, word[base], field[stack[base]])) {
                        ...
                        stack[base]++; /*pop t, push t+1*/
                    }
                    if(stack[base] == nfield) {
                        clear(map, mapt, m, n, word[base], field[stack[base]]);
                        stack[base] = -2; /*pop*/
                        base--; /*back step*/
                        if(base < 0) break; /*Check to not edit out of bound data*/
                        clear(map, mapt, m, n, word[base], field[stack[base]]);
                        stack[base]++; /*next step*/
                        goto recheck;
                    }
            }
...
int valid(char ** map, char ** mapt, int m, int n, char * word, struct Field field) {
    /*Compare the word with map and mapt considering that the space is a wildcard*/
